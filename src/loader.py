...
def prepare_train_eval(local_rank, gpus_per_node, world_size, run_name, train_configs, model_configs, hdf5_path_train):
    ...
    hdf5_path_eval = hdf5_path_train.replace('train', 'eval') if hdf5_path_train else None
    hdf5_path_eval = hdf5_path_eval if hdf5_path_eval and os.path.exists(hdf5_path_eval) else None
    ...
    if cfgs.distributed_data_parallel:
        ...
        num_open = np.sum(targets == -1)
        num_close = np.sum(targets != -1)
        sample_weights = torch.from_numpy(np.where(targets == -1, (1-labeled_ratio) / num_open, labeled_ratio / num_close))
    ...
    Dis = module.Discriminator(cfgs.img_size, cfgs.d_conv_dim, cfgs.d_spectral_norm, cfgs.attention, cfgs.attention_after_nth_dis_block,
                               ...
                               cfgs.normalize_embed, cfgs.softmax_threshold, cfgs.d_init, cfgs.D_depth, cfgs.mixed_precision).to(local_rank)
