...
def calculate_accuracy(dataloader, generator, discriminator, D_loss, num_evaluate, truncated_factor, prior, latent_op,
    ...
    if eval_generated_sample:
        for batch_id in tqdm(range(total_batch), disable=disable_tqdm):
            ...
            with torch.no_grad():
                dis_out_fake = pred_dis_out(discriminator, fake_images, fake_labels, conditional_strategy).detach().cpu().numpy()
                dis_out_real = pred_dis_out(discriminator, real_images, real_labels, conditional_strategy).detach().cpu().numpy()
