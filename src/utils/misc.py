...
def calculate_all_sn(model):
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            if "weight" in name and "bn" not in name and "shared" not in name and "deconv" not in name and "embedding" not in name and "head" not in name:
                if "blocks" in name:
    ...
    with torch.no_grad() if latent_op is False else dummy_context_mgr() as mpc:
        for i in tqdm(range(0, n_batches), disable=False):
            ...
            if is_generate:
                ...
                try:
                    images, labels, *other = next(data_iter)
...
def c(model):
    sigmas = {}
    mods = list(set(['.'.join(name.split('.')[:-1]) for name, param in model.items()]))
    for layer_name in mods:
        if f'{layer_name}.weight' not in model or f'{layer_name}.sv0' not in model or f'{layer_name}.u0' not in model:
            continue
        weight_orig = reshape_weight_to_matrix(model[f'{layer_name}.weight'])
        weight_u = model[f'{layer_name}.u0']
        weight_v = model[f'{layer_name}.sv0']
        print(weight_orig.shape, weight_u.shape, weight_v.shape)
        sigmas[layer_name] = torch.dot(weight_u, torch.mv(weight_orig, weight_v.view(-1, 1)))
    return sigmas
