...
from utils.losses import Conditional_Contrastive_loss, Proxy_NCA_loss, NT_Xent_loss, HLoss
from utils.diff_aug import DiffAugment
from utils.cr_diff_aug import CR_DiffAug
from utils.predict import pred_dis_out
...
LOG_FORMAT = (
    ...
    "Classification_loss: {cls_loss:<.6} "
)
...
class make_worker(object):
    def __init__(self, cfgs, train_configs, model_configs, run_name, best_step, logger, writer, n_gpus, gen_model, dis_model,
        ...
        self.cross_entropy_lambda = cfgs.cross_entropy_lambda
        ...
        if self.conditional_strategy in ['SSGAN', 'OSSSGAN', 'ProjGAN', 'ContraGAN', 'Proxy_NCA_GAN', 'Random', 'Reject']:
        ...
        while step_count <= total_step:
            ...
            for step_index in range(self.d_steps_per_iter):
                ...
                for acml_index in range(self.accumulation_steps):
                    try:
                        real_images, real_labels, *other = next(train_iter)
                    ...
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        elif self.conditional_strategy in ["OSSSGAN", 'Open', 'Reject']:
                            cls_out_real, dis_out_real = self.dis_model(real_images, real_labels)
                            cls_out_fake, dis_out_fake = self.dis_model(fake_images, fake_labels)
                        elif self.conditional_strategy in ["ProjGAN", "no", 'Random', 'Single']:
                        ...
                        if self.conditional_strategy == "ACGAN":
                            ce = (self.ce_loss(cls_out_real, real_labels) + self.ce_loss(cls_out_fake, fake_labels))
                            dis_acml_loss += ce
                        elif self.conditional_strategy in ["SSGAN"]:
                            mask = torch.nonzero(real_labels != -1).squeeze()
                            ce = self.ce_loss(cls_out_real.index_select(0, mask), real_labels.index_select(0, mask))
                            dis_acml_loss += self.cross_entropy_lambda * ce
                        elif self.conditional_strategy in ["OSSSGAN", 'Open', 'Reject']:
                            mask = torch.nonzero(real_labels != -1).squeeze()
                            ratio = mask.shape[0] / self.batch_size
                            ce = self.ce_loss(cls_out_real.index_select(0, mask), real_labels.index_select(0, mask)) + \
                                self.ce_loss(cls_out_fake, fake_labels) * ratio
                            ent = self.ent_loss(cls_out_real) + self.ent_loss(cls_out_fake) * ratio
                            dis_acml_loss += self.cross_entropy_lambda * (ce - ent)
                        elif self.conditional_strategy == "NT_Xent_GAN":
                        ...
                        if self.cr:
                            ...
                            elif self.conditional_strategy in ["SSGAN", "OSSSGAN", "ProjGAN", "no", 'Random', 'Single', 'Open', 'Reject']:
                                dis_out_real_aug = pred_dis_out(self.dis_model, real_images_aug, real_labels, self.conditional_strategy)
                            elif self.conditional_strategy in ["NT_Xent_GAN", "Proxy_NCA_GAN", "ContraGAN"]:
                                _, cls_embed_real_aug, dis_out_real_aug = self.dis_model(real_images_aug, real_labels)
                        ...
                        if self.bcr:
                            ...
                            if self.conditional_strategy == "ACGAN":
                                ...
                                dis_out_real_aug = pred_dis_out(self.dis_model, real_images_aug, real_labels, self.conditional_strategy)
                                dis_out_fake_aug = pred_dis_out(self.dis_model, fake_images_aug, fake_labels, self.conditional_strategy)
                        ...
                        if self.zcr:
                            ...
                            if self.conditional_strategy in ["ACGAN", "Proxy_NCA_GAN", "NT_Xent_GAN", "ContraGAN"]:
                                ...
                                dis_out_fake_zaug = pred_dis_out(self.dis_model, fake_images_zaug, fake_labels, self.conditional_strategy)
                            elif self.conditional_strategy in ["Proxy_NCA_GAN", "NT_Xent_GAN", "ContraGAN"]:
                ...
                for acml_step in range(self.accumulation_steps):
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        if self.conditional_strategy in ["ACGAN"]:
                            ...
                            dis_out_fake = pred_dis_out(self.dis_model, fake_images, fake_labels, self.conditional_strategy)
                        elif self.conditional_strategy in ["NT_Xent_GAN", "Proxy_NCA_GAN", "ContraGAN"]:
            ...
            if step_count % self.print_every == 0 and self.global_rank == 0:
                log_message = LOG_FORMAT.format(step=step_count,
                                                ...
                                                cls_loss=ce.item() if ce else 'no',
                                                )
    ...
    def evaluation(self, step, standing_statistics, standing_step):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            labeled_entropy, unlabeled_close_entropy, unlabeled_open_entropy = calculate_entropy(self.train_dataloader, generator, self.dis_model, self.D_loss, self.num_eval[self.eval_type],
                                                            self.truncated_factor, self.prior, self.latent_op, self.latent_op_step, self.latent_op_alpha,
                                                            self.latent_op_beta, self.local_rank, cr=self.cr, logger=self.logger)

            if self.conditional_strategy in ['SSGAN', 'OSSSGAN', 'ProjGAN', 'ContraGAN', 'Proxy_NCA_GAN', 'Random', 'Reject']:  # 'Single' require N + 1 dim input
                if self.dataset_name == "cifar10":
                    classes_ = torch.tensor([c for c in range(self.num_classes)], dtype=torch.long)
                    labels = ["airplane", "automobile", "bird", "cat", "deer", "dog", "frog", "horse", "ship", "truck"]
                else:
                    if self.num_classes > 10:
                        classes_ = torch.tensor(random.sample(range(0, self.num_classes), 10), dtype=torch.long)
                    else:
                        classes_ = torch.tensor([c for c in range(self.num_classes)], dtype=torch.long)
                    labels = classes_.detach().cpu().numpy()
                classes = torch.cat((torch.eye(self.num_classes)[classes_], torch.ones(1, self.num_classes) / self.num_classes), dim=0).to(self.local_rank)
            ...
            if self.global_rank == 0:
                ...
                self.writer.add_scalars('Entropy', {'labeled entropy': labeled_entropy, 'unlabeled closed entropy': unlabeled_close_entropy, 'unlabeled open entropy': unlabeled_open_entropy}, step)
                self.writer.add_scalars('Open-set entropy', {}, step)
                ...
                self.logger.info('Entropy for labeled set (Step: {step}): {ent}'.format(step=step, ent=labeled_entropy))
                self.logger.info('Entropy for unlabeled closed set (Step: {step}): {ent}'.format(step=step, ent=unlabeled_close_entropy))
                self.logger.info('Entropy for unlabeled open set (Step: {step}): {ent}'.format(step=step, ent=unlabeled_open_entropy))
                if self.train:
    ...
    def run_linear_interpolation(self, nrow, ncol, fix_z, fix_y, standing_statistics, standing_step, num_images=100):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            generator = change_generator_mode(self.gen_model, self.Gen_copy, self.bn_stat_OnTheFly, standing_statistics, standing_step,
                                              self.prior, self.batch_size, self.z_dim, self.num_classes, self.local_rank, training=False, counter=self.counter)
            shared_ = generator.module.shared if isinstance(generator, DataParallel) or isinstance(generator, DistributedDataParallel) else generator.shared
            shared = lambda x: shared_(torch.eye(self.num_classes)[x].to(self.local_rank))
    ...
    def run_tsne(self, dataloader, standing_statistics, standing_step):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            for i in range(num_batches):
                ...
                real_images, real_labels, *other = next(tsne_iter)
                real_images, real_labels = real_images.to(self.local_rank), real_labels.to(self.local_rank)
                fake_images = generator(zs, fake_labels, evaluation=True)

                _ = self.dis_model(real_images, real_labels)
                ...
                _ = self.dis_model(fake_images, fake_labels)
