...
from utils.model_ops import ConditionalBatchNorm2d_for_skip_and_shared, batchnorm_2d, snconv2d, conv2d, linear, snlinear, \
    embedding, sn_embedding, snembeddingent, embeddingent, Self_Attn, init_weights, get_activation_fn
...
class GenBlock(nn.Module):
    def __init__(self, in_channels, out_channels, g_spectral_norm, activation_fn, conditional_bn, z_dims_after_concat,
        ...
        self.conv2d1 = which_conv(in_channels=in_channels, out_channels=self.hidden_channels, kernel_size=1, stride=1, padding=0)
        self.conv2d2 = which_conv(in_channels=self.hidden_channels, out_channels=self.hidden_channels, kernel_size=3, stride=1, padding=1)
        self.conv2d3 = which_conv(in_channels=self.hidden_channels, out_channels=self.hidden_channels, kernel_size=3, stride=1, padding=1)
        self.conv2d4 = which_conv(in_channels=self.hidden_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0)
...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, shared_dim, img_size, g_conv_dim, g_spectral_norm, attention, attention_after_nth_gen_block, activation_fn,
        ...
        self.linear0 = which_linear(in_features=self.z_dims_after_concat, out_features=self.in_dims[0]*self.bottom*self.bottom)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, img_size, d_conv_dim, d_spectral_norm, attention, attention_after_nth_dis_block, activation_fn, conditional_strategy,
        ...
        which_conv = snconv2d if d_spectral_norm else conv2d
        which_linear = snlinear if d_spectral_norm else linear
        which_embedding = sn_embedding if d_spectral_norm else embedding

        self.input_conv = which_conv(in_channels=self.in_dims[0], out_channels=self.out_dims[0], kernel_size=3, stride=1, padding=1)
        ...
        self.linear1 = which_linear(in_features=self.out_dims[-1], out_features=1)
        if self.conditional_strategy in ['ContraGAN', 'Proxy_NCA_GAN', 'NT_Xent_GAN']:
            self.linear2 = which_linear(in_features=self.out_dims[-1], out_features=hypersphere_dim)
            ...
            self.embedding = which_embedding(num_classes, self.out_dims[-1])
        elif self.conditional_strategy == 'ACGAN':
            self.linear4 = which_linear(in_features=self.out_dims[-1], out_features=num_classes)
        else:
            pass
