...
from utils.model_ops import ConditionalBatchNorm2d, batchnorm_2d, snconv2d, conv2d, linear, snlinear, \
    sndeconv2d, deconv2d, \
...
class GenBlock(nn.Module):
    def __init__(self, in_channels, out_channels, g_spectral_norm, activation_fn, conditional_bn, num_classes):
        ...
        which_deconv = sndeconv2d if g_spectral_norm else deconv2d
        self.deconv0 = which_deconv(in_channels=in_channels, out_channels=out_channels, kernel_size=4, stride=2, padding=1)
...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, shared_dim, img_size, g_conv_dim, g_spectral_norm, attention, attention_after_nth_gen_block, activation_fn,
        ...
        self.linear0 = which_linear(in_features=self.z_dim, out_features=self.in_dims[0]*4*4)
        ...
        self.conv4 = which_conv(in_channels=self.out_dims[-1], out_channels=3, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, z, label_, evaluation=False):
...
class Discriminator(nn.Module):
    ...
    def __init__(self, img_size, d_conv_dim, d_spectral_norm, attention, attention_after_nth_dis_block, activation_fn, conditional_strategy,
        ...
        self.linear1 = which_linear(in_features=512, out_features=1)
        if self.conditional_strategy in ['ContraGAN', 'Proxy_NCA_GAN', 'NT_Xent_GAN']:
            self.linear2 = which_linear(in_features=512, out_features=hypersphere_dim)
            ...
            self.embedding = which_embedding(num_classes, 512)
        elif self.conditional_strategy == 'ACGAN':
            self.linear4 = which_linear(in_features=512, out_features=num_classes)
        else:
            pass
